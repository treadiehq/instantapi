  ...
  console.<span class="text-blue-300">log</span>(<span class="text-green-300">'Received:'</span>, data);
            ...
            <Teleport to="body">
              ...
              <div v-if="isAuthenticated" class="lg:col-span-1">
                ...
                <div v-if="mode === 'snippet'" class="border bg-gray-500/5 border-t-0 border-gray-500/15 overflow-hidden">
                  <div class="p-4 border-b border-gray-500/10 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                      <div class="w-2 h-2 rounded-full" :class="selectedExampleIndex !== null ? 'bg-green-300 animate-pulse' : 'bg-gray-500'"></div>
                  ...
                  <div class="min-h-[300px] flex flex-col">
                    <div v-if="selectedExampleIndex === null" class="flex-1 flex flex-col items-center justify-center p-6 text-center">
                      <div class="w-12 h-12 rounded-full bg-gray-500/10 flex items-center justify-center mb-4">
                      ...
                      <p class="text-gray-400 text-sm">Select an example to see output</p>
                    </div>
                    <div v-else-if="currentExampleOutput?.type === 'image'" class="flex-1 flex flex-col">
                      <div class="flex-1 bg-gradient-to-br from-orange-300/20 via-pink-300/20 to-purple-300/20 p-4 flex items-center justify-center">
                        <img :src="currentExampleOutput.preview" :alt="currentExampleOutput.description" class="max-w-full max-h-[180px] rounded-lg shadow-xl object-cover" />
                      </div>
                      <div class="p-3 border-t border-gray-500/10 bg-black/50">
                        <p class="text-white text-xs font-medium">{{ currentExampleOutput.description }}</p>
                    ...
                    <div v-else-if="currentExampleOutput?.type === 'chat'" class="flex-1 flex flex-col">
                      <div class="flex-1 p-3 space-y-3 overflow-y-auto">
                        <div class="flex gap-2">
                          <div class="w-6 h-6 rounded-full bg-blue-300/20 flex items-center justify-center shrink-0">
                            <svg class="w-3 h-3 text-blue-300" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                            </svg>
                          </div>
                          <div class="flex-1">
                            <p class="text-xs text-gray-500 mb-0.5">You</p>
                            <p class="text-xs text-gray-300">{{ currentExampleOutput.prompt }}</p>
                          </div>
                        </div>
                        <div class="flex gap-2">
                          <div class="w-6 h-6 rounded-full bg-green-300/20 flex items-center justify-center shrink-0">
                            <svg class="w-3 h-3 text-green-300" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                            </svg>
                          </div>
                          <div class="flex-1">
                            <p class="text-xs text-gray-500 mb-0.5">{{ currentExampleOutput.model }}</p>
                            <p class="text-xs text-gray-200 leading-relaxed">{{ currentExampleOutput.response }}</p>
                          </div>
                        </div>
                      </div>
                      <div class="p-2 border-t border-gray-500/10 bg-black/50 flex items-center justify-between">
...
const exampleOutputs = computed(() => {
  const jsOutputs = [
    // Hello
    { type: 'json', description: 'Simple greeting response', latency: '23ms', data: JSON.stringify({ result: { message: "Hello, Dante!", timestamp: "2025-01-15T10:30:00.000Z" }, durationMs: 23 }, null, 2) },
    // AI Agent (OpenAI)
    { type: 'chat', model: 'gpt-4o-mini', prompt: 'What is the capital of France?', response: 'The capital of France is Paris. Paris is not only the political capital but also the cultural, economic, and historical heart of France.', tokens: '67 tokens', latency: '892ms' },
    // AI Agent (Anthropic)
    { type: 'chat', model: 'claude-3-5-sonnet', prompt: 'Explain quantum computing in simple terms', response: 'Imagine a regular computer as a person who can only read one book at a time. A quantum computer is like someone who can read all the books in a library simultaneously!', tokens: '89 tokens', latency: '1,247ms' },
    // Image Generator (DALL-E)
    { type: 'image', model: 'dall-e-3', description: 'A futuristic city with flying cars at sunset', preview: 'https://images.unsplash.com/photo-1480714378408-67cf0d13bc1b?w=400&h=300&fit=crop&auto=format' },
    // Web Scraper
    { type: 'json', description: 'Extracted page data', latency: '156ms', data: JSON.stringify({ result: { url: "https://example.com", title: "Example Domain", linkCount: 1 }, durationMs: 156 }, null, 2) },
    // Fetch API
    { type: 'json', description: 'GitHub Zen quote', latency: '89ms', data: JSON.stringify({ result: { url: "https://api.github.com/zen", data: "Responsive is better than fast." }, durationMs: 89 }, null, 2) },
    // Webhook
    { type: 'json', description: 'Webhook processed', latency: '12ms', data: JSON.stringify({ result: { received: true, event: "user.created" }, durationMs: 12 }, null, 2) }
  ]
  const pyOutputs = [
    { type: 'json', description: 'Simple greeting response', latency: '45ms', data: JSON.stringify({ result: { message: "Hello, Dante!" }, durationMs: 45 }, null, 2) },
    { type: 'chat', model: 'gpt-4o-mini', prompt: 'What is the capital of France?', response: 'The capital of France is Paris.', tokens: '67 tokens', latency: '934ms' },
    { type: 'chat', model: 'claude-3-5-sonnet', prompt: 'Explain quantum computing', response: 'Quantum computers use qubits instead of regular bits.', tokens: '89 tokens', latency: '1,312ms' },
    { type: 'image', model: 'dall-e-3', description: 'A futuristic city at sunset', preview: 'https://images.unsplash.com/photo-1480714378408-67cf0d13bc1b?w=400&h=300&fit=crop&auto=format' },
    { type: 'json', description: 'Extracted page data', latency: '203ms', data: JSON.stringify({ result: { url: "https://example.com", title: "Example Domain" }, durationMs: 203 }, null, 2) },
    { type: 'json', description: 'GitHub Zen quote', latency: '112ms', data: JSON.stringify({ result: { data: "Mind your words." }, durationMs: 112 }, null, 2) },
    { type: 'json', description: 'Webhook processed', latency: '18ms', data: JSON.stringify({ result: { received: true }, durationMs: 18 }, null, 2) }
  ]
